<template>
  <div class="lotteryNotice">
    <div v-for="(item,index) in activityData" :key="index" class="clubActivity-item" @click="toNotic(item)">
      <activity-noitce v-if="item.resType=='activity'&&isType !=='topic'" :item="item.feedData"></activity-noitce>
      <topic-noitce v-if="item.resType=='topic'||isType=='topic'" :item="item.feedData"></topic-noitce>
      <myDashed></myDashed>
      <!-- 底部bar -->
      <div v-if="!card" class="tip-list" ref="tip">
        <div v-if="item.resType=='activity'&&isType !=='topic'" @click="push2('/views/activity/active_layout/active_detail',{resId:item.resId})" class="comment">
          ...
          <text class="icon-text">{{item.commentNum?item.commentNum:item.feedData.commentNum}}</text>
        </div>
        ...
        <div @click="thumb(item)" class="thumb">
          <image class="img image-view" :src='imgUrl+(item.isThumb?item.isThumb:item.feedData.isThumb==1?"thumbNumActive.png":"thumbNum.png")' ></image>
          <text class="icon-text">{{item.thumbNum?item.thumbNum:item.feedData.thumbNum}}</text>
        </div>
        <div @click="share(item)" class="share">
          <image class="img image-view" :src='imgUrl+"min-share.png"' ></image>
          <!-- <text class="icon-text">{{item.shareNum?item.shareNum:item.feedData.shareNum}}</text> -->
        </div>
        ...
        <!-- <div v-if="activeType" class="joinNum">
          <text class="font-22 join-text">想参加{{item.joinNum?item.joinNum:item.feedData.joinNum}}人</text>
        </div> -->
      </div>
...
export default {
  ...
  methods: {
    ...
    show(item) {
      console.log(item);
      this.key = true;
      this.iMobile = item.feedData.mobile;
      this.resId = item.resId;
      this.item = item;
    },
    // 编辑活动
    edit() {
      if (this.isType === "topic") {
    ...
    activeState(item) {
      let itime = new Date();
      let stime = new Date(
        item.feedData.starttime.replace(new RegExp(/-/gm), "/")
      );
      let etime = new Date(
        item.feedData.endtime.replace(new RegExp(/-/gm), "/")
      );
    ...
    thumb(item) {
      ...
      if (!this.mobile || this.mobile === "") {
        ...
        toLogin({ loginOut: "0" });
        return;
      } else if (item.isThumb ? item.isThumb : item.feedData.isThumb == 1) {
        // 取消点赞
        modal.confirm(
          {
            message: "确定取消点赞么 ?",
              ...
              deleteThumb(
                ...
                .then(res => {
                  if (res.status == 0) {
                    item.thumbNum
                      ? item.thumbNum
                      : (item.feedData.thumbNum -= 1);
                    item.isThumb ? item.isThumb : (item.feedData.isThumb = "0");
                  } else {
        ...
        if (!this.isShow) {
          ...
          putThumb(
            ...
            .then(res => {
              this.isShow = false;
              if (res.status == 0) {
                item.thumbNum ? item.thumbNum : (item.feedData.thumbNum += 1);
                item.isThumb ? item.isThumb : (item.feedData.isThumb = "1");
              } else {
                modal.toast({ message: res.desc, duration: 1 });
              }
            })
    ...
    share(item) {
      ...
      if (_this.isType === "topic" || item.resType === "topic") {
        let userIcon = item.feedData.user.icon;
        sObj = {
          title: "分享话题：" + item.feedData.topicTags[0],
          desc: item.feedData.content,
          web_url:
            _this.commonUrl +
        ...
        sObj = {
          title: "分享活动：" + item.feedData.title,
          desc: item.feedData.content,
          web_url:
            _this.commonUrl +
            "myyuleWeb/webShare/activity/index.html?resId=" +
            item.resId,
          img_url: "http://media2.myyule.cn/" + item.feedData.logo,
          sourse: "myyule"
        };
      }
      sharePage(sObj);
