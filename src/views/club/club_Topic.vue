<template>
  <div class="lotteryNotice">
    <div v-for="(item,index) in activityData" :key="index" class="clubActivity-item">
      <topic-noitce @click="push2('/views/topic/topic_detail',{resId:item.resId})" :item="item"></topic-noitce>
      <myDashed></myDashed>
      <!-- 底部bar -->
      <div v-if="!card" class="tip-list" ref="tip">
        <div @click="push2('/views/topic/topic_detail',{resId:item.resId})" class="comment">
        ...
        <div @click="thumb(item)" class="thumb">
          <image class="img image-view" :src='imgUrl+(item.isThumb==1?"thumbNumActive.png":"thumbNum.png")' ></image>
          <text class="icon-text">{{item.thumbNum}}</text>
        </div>
        <div @click="share(item)" class="share">
          <image class="img image-view" :src='imgUrl+"min-share.png"' ></image>
          <!-- <text class="icon-text">{{item.shareNum?item.shareNum:item.shareNum}}</text> -->
        </div>
        ...
        <!-- <div v-if="activeType" class="joinNum">
          <text class="font-22 join-text">想参加{{item.joinNum?item.joinNum:item.joinNum}}人</text>
        </div> -->
      </div>
...
import topicNoitce from "../../views/myPlace/topicNoitce.vue";
const modal = weex.requireModule("modal");
export default {
  mixins: [mixins],
  components: { myDashed, WxcMask, WxcPopup, topicNoitce },
  props: {
    activityData: {},
    card: {},
    isType: {}
          ...
          putReport(
            {
              ...
              url: this.commonUrl + "/topic_detail?resId=" + this.resId
            },
            { token: this.mobile }
          )
            .then(res => {
